...
// Adicionado 'isInitial' para sabermos qual é a carta marco-zero
...
const CompactDisc = ({ large = false }) => (
  <div className={`${large ? 'w-40 h-40 border-[14px]' : 'w-24 h-24 border-[8px]'} rounded-full border-zinc-950 bg-gradient-to-tr from-purple-900 via-fuchsia-700 to-violet-900 shadow-[0_0_30px_rgba(168,85,247,0.3)] animate-[spin_4s_linear_infinite] flex items-center justify-center relative overflow-hidden flex-shrink-0`}>
    ...
    <div className={`${large ? 'w-10 h-10 border-4' : 'w-6 h-6 border-2'} rounded-full bg-zinc-950 border-zinc-800 z-10 shadow-inner`}></div>
  </div>
);
...
export default function TVPage() {
  ...
  const criarSala = () => {
    ...
    if (savedDeck.length < 10) { alert("Importe pelo menos 10 músicas!"); return; }
  ...
  // MOTOR DE SCROLL MATEMÁTICO (Impede a barra e o topo de mexerem)
  useEffect(() => {
  ...
  const iniciarPartidaGlobal = () => {
    ...
    // Adiciona o selo de "isInitial" na primeira carta de cada um
  ...
  // Repare no h-screen e w-screen para garantir que o contêiner pai nunca scrolle!
  return (
    <div className="flex flex-col h-screen w-screen bg-zinc-950 text-white font-sans overflow-hidden relative">
      ...
      <style jsx>{`
        ...
        .smooth-text { transition: all 0.5s cubic-bezier(0.4, 0, 0.2, 1); }
      ...
      <div className="w-full bg-zinc-900/40 backdrop-blur-md px-8 py-4 flex justify-between items-center border-b border-zinc-800/50 z-50 flex-shrink-0">
        ...
        <div className="flex items-center gap-6">
          ...
          <button onClick={() => signOut()} className="text-[10px] font-bold text-zinc-500 hover:text-white transition-colors">SAIR</button>
        </div>
      </div>
      ...
      {!gameStarted ? (
        <div className="flex-1 flex flex-col items-center justify-center p-10 z-10">
          {!roomCode ? (
            <button onClick={criarSala} className="bg-white text-black font-black text-3xl py-6 px-16 rounded-full shadow-2xl hover:scale-105 transition-transform active:scale-95">ABRIR NOVA SALA</button>
          ) : (
            <div className="w-full max-w-4xl text-center">
              <h1 className="text-[10rem] leading-none font-black text-white mb-16 tracking-tighter animate-bounce">{roomCode}</h1>
              <div className="grid grid-cols-2 gap-4">
                {players.map((p) => (
                  <div key={p.name} className={`p-4 rounded-2xl border-2 ${p.isReady ? 'border-green-500 bg-green-500/5' : 'border-zinc-800 bg-zinc-900/50'} smooth-text`}>
        ...
        <div className="flex flex-1 relative overflow-hidden z-10">
          ...
          <div className="flex-1 flex flex-col relative overflow-hidden">
            ...
            <div className={`w-full py-8 text-center transition-all duration-700 z-20 flex-shrink-0 ${actionState === 'revealed' ? (revealSuccess ? 'bg-green-600 shadow-[0_20px_50px_rgba(22,163,74,0.3)]' : 'bg-red-600 shadow-[0_20px_50px_rgba(220,38,38,0.3)]') : 'bg-transparent'}`}>
              <h1 className="text-5xl font-black uppercase tracking-tighter smooth-text">
            ...
            {/* REMOVIDO "Escute a TV" ABAIXO DO CD */}
            {actionState === 'waiting' && (
              <div className="absolute top-[45%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-10 animate-pulse">
                <CompactDisc large />
              </div>
            )}

            {/* LINHA DO TEMPO (Com ID para o scroll matemático funcionar perfeito) */}
              ...
              <div className="flex items-end h-[24rem] w-max px-[40vw] gap-4 mx-auto">
                ...
                {players[currentPlayerIndex]?.timeline.map((track, i) => (
                  <Fragment key={track.id}>
                    {/* SLOT */}
                    <div id={`slot-${i}`} className={`flex flex-col items-center justify-end relative h-full transition-all duration-700 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${mobileAction === i ? 'w-48' : 'w-4'}`}>
                      {mobileAction === i && (
                        <div className={`absolute bottom-20 z-30 ${actionState === 'slotted' ? 'anim-drop' : ''} ${actionState === 'revealed' && revealSuccess ? 'anim-flip' : ''} ${actionState === 'revealed' && !revealSuccess ? 'anim-fall' : ''}`}>
                          {actionState === 'revealed' && revealSuccess ? (
                            <div className="flex flex-col items-center scale-110">
                              ...
                              {/* Nome da música e artista inserido para a carta que acabou de virar */}
                              <div className="mt-4 text-center w-40 animate-fade-in">
                    ...
                    {/* CARTA EXISTENTE DA TIMELINE */}
                    <div className="flex flex-col items-center flex-shrink-0 z-20 group">
                      <div className="bg-zinc-100 text-black font-black text-3xl px-6 py-1 rounded-full mb-[-1rem] z-20 shadow-xl group-hover:scale-110 transition-transform">{track.year}</div>
                      
                      {/* Outline Dourado Dinâmico para a "Carta Inicial" */}
                      <img src={track.imageUrl} className={`w-44 h-44 rounded-[2.5rem] border-4 shadow-2xl object-cover transition-all ${track.isInitial ? 'border-amber-400 shadow-[0_0_40px_rgba(251,191,36,0.3)] group-hover:border-amber-300' : 'border-zinc-800 group-hover:border-zinc-500'}`} />
                      
                      <div className="mt-4 text-center w-44 animate-fade-in">
                ...
                {/* ÚLTIMO SLOT DA PONTA DIREITA */}
                <div id={`slot-${players[currentPlayerIndex]?.timeline.length}`} className={`flex flex-col items-center justify-end relative h-full transition-all duration-700 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${mobileAction === players[currentPlayerIndex]?.timeline.length ? 'w-48' : 'w-4'}`}>
